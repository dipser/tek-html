...
function compact(src, out, callback, debug) {
    var dir = resolve(src, '..');
        ...
        doc.find('link[rel="stylesheet"]').each(function () {
            ...
            jobQueue.push(function (next) {
                ...
                createStyle(toFullPath(href), function (style) {
                    $link.after(style);
                    $link.remove();
                    if (debug)console.log('\tinline done');
                    next();
                });
            });
        });
...
module.exports = function (src, out, callback, debug) {
    if (!exists) {
